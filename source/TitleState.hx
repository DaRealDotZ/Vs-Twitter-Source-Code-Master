...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		blackScreen = new FlxSprite().makeGraphic(FlxG.width, FlxG.height,  FlxColor.fromRGB(6, 196, 207));
		...
		sussyTeam = new FlxSprite(0, FlxG.height * 0.4).loadGraphic(Paths.image('Team_BlueBird'));
		add(sussyTeam);
		sussyTeam.visible = false;
		sussyTeam.setGraphicSize(Std.int(sussyTeam.width * 0.55));
		sussyTeam.updateHitbox();
		sussyTeam.screenCenter();
		sussyTeam.antialiasing = ClientPrefs.globalAntialiasing;
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			switch (curBeat)
			{
				...
				case 3:
					addMoreText('', 45);
					sussyTeam.visible = true;
					FlxTween.tween(sussyTeam, {y: sussyTeam.y + 50}, 0.6, {ease: FlxEase.quadInOut, type: PINGPONG});
					addMoreText('', 45);
				// credTextShit.text += '\npresent...';
				...
				// credTextShit.text += '\nNight';
				case 15:
					addMoreText('Drama'); // credTextShit.text += '\nFunkin';
